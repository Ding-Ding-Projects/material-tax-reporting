...
Their hashes and the source commit are recorded in generated build provenance. A later package command refuses output whose commit or hashes do not match.
...
Run `build-installer.bat` with the same silent options when required. It repeats the real application build from cleared output, generates a multi-resolution ICO from `assets/brand/material-tax-reporting-mark.png`, clears stale package output, and runs electron-builder's Squirrel.Windows target with publication disabled.
